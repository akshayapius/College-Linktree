# College Linktree

A link tree which helps in creating a link in which the user can display all of their required links to blogs, websites and social media pages. It organises and creates a structured view of all links relating to an individual, or in our case, organization. The link showcases various icons at first glance of the various platforms to which it is providing a link. On clicking the icons, it is redirected to the respective page. It thus helps neatly organise all of the user’s crucial links into a single page.

In our link tree, which was created for a college, we provide all of the colleges major media pages, contact information, clubs hosted and links to the various departments and their respective websites. It provides an aesthetic and pleasing website with easy to use buttons, to easily obtain all the links of the college.

The website is made using HTML and CSS, deploying HTML features such as Bootstrap and Flexbox.

## Acknowledgements

 - [Easy-to-use stickers](https://flaticon.com)
 - [HD wallpapers](https://wallpapers.com)
 - [For language based queries](https://w3schools.com)
 - [HTML and CSS tutorials](https://www.youtube.com/results?search_query=brototype)

## Authors

- [@angelsara](https://www.github.com/Angel2526)
- [@ashnaregi](https://www.github.com/Ashna2002)
- [@alphonsasiby](https://www.github.com/Alphs28)
- [@akshayapius](https://www.github.com/akshayapius)

## Documentation

The website's main page contains the following:
- College logo
- Name of College
- Icons for:
    - Instagram
    - Facebook
    - Twitter
    - LinkedIn
- Contact:
    - Address
    - Phone Number
    - Email
    - Admission
- Communtity:
    - IEDC
    - IEEE
    - YI-YUVA
    - TinkerHub
    - GDSC
    - NSS
- Departments:
    - Computer Science and Engineering Department
    - Artificial Intelligence and Data Science Department
    - Mechanical Engineering Department
    - Civil Engineering Department
    - Electrical and Electronics Engineering Department
    - Electronics and Communication Engineering Department
- Feedback
- Footer


## Features

- Feedback Form
- Pictorial representation of buttons
- Footer
- Aesthetically Pleasing


index<br>
<img width="960" alt="image" src="https://user-images.githubusercontent.com/91876904/208283409-b0169f50-18fe-460b-b5aa-9143187d3423.png">
<img width="960" alt="image" src="https://user-images.githubusercontent.com/91876904/208283425-3166d1fc-bcc8-48a4-847f-7067bdcc07c6.png">
contact
<img width="960" alt="image" src="https://user-images.githubusercontent.com/91876904/208283433-b93678dc-a689-4ade-be72-62f353654d8a.png">
community
<img width="960" alt="image" src="https://user-images.githubusercontent.com/91876904/208283459-4a933993-df9a-416e-91a1-8a2d3a598a51.png">
department
<img width="960" alt="image" src="https://user-images.githubusercontent.com/91876904/208283464-9edbdd66-f2ac-4ce1-b40c-266926f1b5d2.png">
cse
<img width="960" alt="image" src="https://user-images.githubusercontent.com/91876904/208283521-2ce5db01-adc3-4558-b21f-1d72b97bc8f7.png">
feedback
<img width="960" alt="image" src="https://user-images.githubusercontent.com/91876904/208283577-f6f0c448-71c5-472b-8dad-d5092e41de33.png">
<img width="960" alt="image" src="https://user-images.githubusercontent.com/91876904/208283594-5a2f519a-663b-444e-b4e4-aa1126be5b12.png">





